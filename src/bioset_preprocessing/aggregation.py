...
class HierarchicalAggregator:
    ...
    def aggregate(self) -> List[HierarchyLevel]:
        ...
        for level in range(self.n_levels):
            ...
            hierarchy_level = HierarchyLevel(level=level, tile_size_x=tile_size_x, tile_size_y=tile_size_y)
            ...
            for (region_x, region_y), tile_results in regions.items():
                self._aggregate_region(hierarchy_level, level, region_x, region_y, scale, tile_results)
            levels.append(hierarchy_level)
        return levels

    def _aggregate_region(self, hierarchy_level, level, region_x, region_y, scale, tile_results):
        if not tile_results:
            return
        ...
        channel_agg = defaultdict(lambda: {"voxel_count": 0, "sum_intensity": 0.0})
        ...
        for (ch, r), agg in channel_agg.items():
            hierarchy_level.channels.append(AggregatedChannelStats(
                channel=ch, r_um=r, hierarchy_level=level,
                tile_x0=tile_x0, tile_x1=tile_x1, tile_y0=tile_y0, tile_y1=tile_y1,
                voxel_count=agg["voxel_count"], sum_intensity=agg["sum_intensity"],
            ))

        pair_agg = defaultdict(lambda: {"a_vox": 0, "b_vox": 0, "inter_vox": 0, "union_vox": 0})
        for result in tile_results:
        ...
        for (a, b, r), agg in pair_agg.items():
            hierarchy_level.pairs.append(AggregatedPair(
                a=a, b=b, r_um=r, hierarchy_level=level,
                tile_x0=tile_x0, tile_x1=tile_x1, tile_y0=tile_y0, tile_y1=tile_y1,
                a_vox=agg["a_vox"], b_vox=agg["b_vox"],
                inter_vox=agg["inter_vox"], union_vox=agg["union_vox"],
            ))

        set_agg = defaultdict(lambda: {"member_voxels": defaultdict(int), "inter_vox": 0, "union_vox": 0})
        for result in tile_results:
        ...
        for (members, r), agg in set_agg.items():
            hierarchy_level.sets.append(AggregatedSet(
                members=members, r_um=r, hierarchy_level=level,
                tile_x0=tile_x0, tile_x1=tile_x1, tile_y0=tile_y0, tile_y1=tile_y1,
