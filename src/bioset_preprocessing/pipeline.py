...
class Pipeline:
    def __init__(self, cfg: PipelineConfig):
        ...
        if cfg.input_mode == "mcmicro":
            self._init_mcmicro_mode(cfg)
        else:
            self._init_legacy_mode(cfg)
    ...
    def _init_legacy_mode(self, cfg: PipelineConfig) -> None:
        """Initialise for legacy (raw zarr + thresholding) mode."""
        ...
        if not self.pyr_local and not self.pyr_remote:
            raise ValueError("Provide at least one of zarr_path or zarr_url.")
        ...
        print(f"[Pipeline] Mode: LEGACY")
        print(f"[Pipeline] High res: {hi_source} component={self.comp_hi} shape={self.A_hi.shape}")
        print(f"[Pipeline] Global threshold: {global_source} component={self.comp_lo} shape={self.A_lo.shape}")
    ...
    def _init_mcmicro_mode(self, cfg: PipelineConfig) -> None:
        """Initialise for MCMICRO (segmentation + quantification) mode."""
        from .adapters.mcmicro_adapter import MCMICROAdapter

        # Open the converted zarr for reading tile intensities
        zarr_path = cfg.mcmicro_zarr_path or cfg.zarr_path
        if not zarr_path:
            raise ValueError("MCMICRO mode requires mcmicro_zarr_path or zarr_path")

        self.pyr_local = ZarrPyramid.open(zarr_path)
        self.pyr_remote = None
        self.comp_hi, self.A_hi = self.pyr_local.highest_res()
        # In MCMICRO mode we don't need low-res for global thresholds
        self.comp_lo, self.A_lo = self.comp_hi, self.A_hi

        print(f"[Pipeline] Mode: MCMICRO")
        print(f"[Pipeline] Zarr source: {zarr_path} shape={self.A_hi.shape}")

        # Initialise adapter
        self._adapter = MCMICROAdapter(
            segmentation_path=cfg.mcmicro_segmentation_path,
            quantification_path=cfg.mcmicro_quantification_path,
            marker_thresholds_path=cfg.marker_thresholds_path,
            channel_names=cfg.channel_names or None,
        )
        self._adapter.load()

        # Override channels from adapter if not explicitly set
        if not cfg.channels:
            cfg.channels = self._adapter.channel_indices

        # Legacy stages not needed in MCMICRO mode
        self.th = None
        self.cc = None
    ...
    def compute_global_thresholds(self) -> Dict[int, float]:
        """Compute global per-channel thresholds from low-res volume.

        Only used in legacy mode.  In MCMICRO mode this is a no-op.
        """
        if self.cfg.input_mode == "mcmicro":
            print("[Pipeline] MCMICRO mode: skipping global threshold computation")
            return {}
    ...
    def _process_single_tile_legacy(
        self,
        tile: TileIndex,
        radii: List[float],
        z: int,
    ) -> OverlapTileResult:
        """Process a single tile using legacy thresholding + CC pipeline."""
        ...
        marker_vox, sum_intensity = self._sync_scalars(
            marker_vox_gpu, sum_intensity_gpu, radii
        )
        del marker_vox_gpu, sum_intensity_gpu
    ...
    def _process_single_tile_mcmicro(
        self,
        tile: TileIndex,
        radii: List[float],
        z: int,
    ) -> OverlapTileResult:
        """Process a single tile using MCMICRO adapter masks."""
        tile_y, tile_x = self.cfg.tile_xy
        ys, xs = tile_slices(tile, tile_y, tile_x)
        ...
        # Get base masks from adapter (channel_idx -> bool mask)
        base_masks_gpu = self._adapter.get_tile_masks(ys, xs, device="gpu")

        # Read intensity volume for this tile (for sum_intensity stats)
        channels = sorted(base_masks_gpu.keys())
        intensity_vol = {}
        for ch_batch in chunked(channels, self.cfg.channel_batch):
            vol_cpu = self.A_hi[0, ch_batch, :, ys, xs].compute()
            vol_gpu = cp.asarray(vol_cpu)
            for i, ch in enumerate(ch_batch):
                intensity_vol[ch] = vol_gpu[i]
            del vol_gpu

        # Dilate each channel's base mask and build the full masks dict
        masks: Dict[float, Dict[int, cp.ndarray]] = {r: {} for r in radii}
        marker_vox_gpu: Dict[float, Dict[int, cp.ndarray]] = {r: {} for r in radii}
        sum_intensity_gpu: Dict[float, Dict[int, cp.ndarray]] = {r: {} for r in radii}

        for ch in channels:
            base_mask = base_masks_gpu[ch]

            dilres: DilationResult = self.dil(base_mask)
            ...
            for r in radii:
                ...
                if r == radii[0]:
                    v = intensity_vol.get(ch)
                    if v is not None and cp.any(m):
                        sum_intensity_gpu[r][ch] = cp.sum(v[m])
                    else:
                        sum_intensity_gpu[r][ch] = cp.asarray(0.0)
                else:
        ...
        del base_masks_gpu, intensity_vol
    ...
    def _process_single_tile(
        ...
        """Route to the appropriate tile processor based on input_mode."""
        if self.cfg.input_mode == "mcmicro":
            return self._process_single_tile_mcmicro(tile, radii, z)
        return self._process_single_tile_legacy(tile, radii, z)

    def _sync_scalars(
        self,
        marker_vox_gpu: Dict[float, Dict[int, cp.ndarray]],
        sum_intensity_gpu: Dict[float, Dict[int, cp.ndarray]],
        radii: List[float],
    ) -> Tuple[Dict[float, Dict[int, int]], Dict[float, Dict[int, float]]]:
        """Batch-sync GPU scalars to CPU in one pass."""
        gpu_scalars = []
        scalar_keys = []

        channels = sorted(next(iter(marker_vox_gpu.values())).keys()) if marker_vox_gpu else []
        ...
        return marker_vox, sum_intensity

    # ------------------------------------------------------------------
    # Stage 1: Tile processing with checkpointing
    # ------------------------------------------------------------------

    def run_tile_processing(self, resume: bool = True) -> int:
        """
        STAGE 1 (GPU): Process tiles and save checkpoints.

        Resumable via tile-level checkpointing.  In MCMICRO mode, global
        threshold computation is skipped.
        """
        if self.cfg.input_mode == "legacy" and not self._t_global:
        ...
        print(f"[Pipeline] Mode: {self.cfg.input_mode}")
    ...
    # ------------------------------------------------------------------
    # Stage 2: Aggregation and export
    # ------------------------------------------------------------------
    ...
    def iter_tile_outputs(self) -> Iterator[TileOutput]:
        """Legacy iterator for per-channel tile outputs (legacy mode only)."""
        if self.cfg.input_mode == "mcmicro":
            raise NotImplementedError(
                "iter_tile_outputs() is not supported in MCMICRO mode. "
                "Use run_tile_processing() instead."
            )
    ...
    def iter_tile_overlap_outputs(self) -> Iterator[OverlapTileResult]:
        """Iterate over tile results (no checkpointing)."""
        if self.cfg.input_mode == "legacy" and not self._t_global:
            self.compute_global_thresholds()
