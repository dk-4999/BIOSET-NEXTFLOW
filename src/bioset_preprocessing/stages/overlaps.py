        ...
        *,
        ...
        for r in self.radii:
            ...
            for ch in channels:
                ...
                channel_stats.append(ChannelTileStats(
                    tile_x=tile_x, tile_y=tile_y, channel=ch, r_um=r,
                    voxel_count=vox_count, sum_intensity=sum_intensity[r][ch],
                ))
        ...
        if n_active < 2:
            return OverlapTileResult(
                tile_x=tile_x, tile_y=tile_y, tile_z=0,
                tile_shape=tile_shape, total_voxels=total_voxels,
                radii_um=list(self.radii), marker_vox=marker_vox,
                channel_stats=channel_stats, pairs=[], sets=[],
                n_active_channels=n_active, n_frequent_pairs=0,
            )
        ...
        freq_pair_data: Dict[Tuple[int, int], Dict] = {}
        ...
        for i, (a, b) in enumerate(all_pairs):
            ...
            if inter >= ms_pair_max:
                ...
                freq_pair_data[pair] = {
                    'inter': inter, 'union': int(union_counts_cpu[i]),
                }
        ...
        for r in self.radii_desc:
            pairs_at_r = [(a, b) for (a, b) in freq_pairs if a in active[r] and b in active[r]]
            if not pairs_at_r:
            ...
            if r == self.r_max:
                for (a, b) in pairs_at_r:
                    ...
                    all_pairs_out.append(PairRow(
                        tile_x=tile_x, tile_y=tile_y, r_um=float(r),
                        a=a, b=b, a_vox=a_vox, b_vox=b_vox,
                        inter_vox=inter, union_vox=uni, iou=iou, overlap_coeff=oc,
                    ))
            else:
        ...
        for r in self.radii_desc:
            sets_at_r = [comb for comb in cand_sets if all(ch in active[r] for ch in comb)]
            if not sets_at_r:
                if self.aggressive_stop_on_fail:
                    break
                continue
            ...
            for i, comb in enumerate(sets_at_r):
                ...
                all_sets_out.append(SetRow(
                    tile_x=tile_x, tile_y=tile_y, r_um=float(r),
                    k=len(comb), members=comb, inter_vox=inter, union_vox=uni,
                    iou=float(iou), overlap_coeff=float(overlap_coeff),
                ))
        ...
        return OverlapTileResult(
            ...
            tile_shape=tile_shape, total_voxels=total_voxels,
            radii_um=list(self.radii), marker_vox=marker_vox,
            channel_stats=channel_stats, pairs=all_pairs_out, sets=all_sets_out,
            n_active_channels=n_active, n_frequent_pairs=n_freq_pairs,
        )
